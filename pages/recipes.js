...
const Recipes = ({ recipes }) => {
	...
	const sortR = (sortROption = "totalTime") => {
		...
		const prepTime = () => {
			switch (sortDir) {
				case "asc":
					return function (a, b) {
						if (!a?.prepTime?.prepHours || !b?.prepTime?.prepHours) return 0;
		...
		const cookTime = () => {
			switch (sortDir) {
				case "asc":
					return function (a, b) {
						if (!a?.cookTime?.cookHours || !b?.cookTime?.cookHours) return 0;
		...
		const totalTime = () => {
			switch (sortDir) {
				case "asc":
					return function (a, b) {
						if (!a?.totalTime?.totalHours || !b?.totalTime?.totalHours)
							return 0;
	...
	useEffect(() => {
		...
		const filt = recipes.reduce(function (result, rec) {
	...
	useEffect(() => {
		const resSort = (a, b) => {
			return a.title
				.charAt(0)
				.toLowerCase()
				.includes(search.charAt(0).toLowerCase())
				? -1
				: 1;
		};

		const searchRes = recipes.reduce(function (result, rec) {
			if (rec.title.toLowerCase().includes(search.toLowerCase()))
				result.push(rec);
